...
static dng_error_code dng_validate(const char* filename)
{
    ...
    try
    {
...

        {
            ...
            printf("fMainIndex %d, fMaskIndex %d, fDepthIndex %d, fEnhancedIndex %d\n", info.fMainIndex, info.fMaskIndex,
                info.fDepthIndex, info.fEnhancedIndex);
            size_t ifd_cnt = info.fIFD.size();
            for (size_t i = 0; i < ifd_cnt; i++)
            {
                printf("width %d, height %d\n", info.fIFD[i]->fImageWidth, info.fIFD[i]->fImageLength);
                printf("is baseline jpeg: %d, can read: %d\n", info.fIFD[i]->IsBaselineJPEG(), info.fIFD[i]->CanRead());
                if (info.fIFD[i]->CanRead() && info.fIFD[i]->IsBaselineJPEG())
                {
                    uint32_t samples = info.fIFD[i]->fSamplesPerPixel;
                    uint32_t bits = info.fIFD[i]->fBitsPerSample[0];
                    uint32_t max_val = (1U << bits) - 1;
                    uint32_t sample_size = 0;

                    dng_image* preview_img = NULL;
                    if (bits <= 8)
                    {
                        preview_img = host.Make_dng_image(dng_rect(info.fIFD[i]->fImageLength, info.fIFD[i]->fImageWidth), 3, ttByte);
                        sample_size = 1;
                    }
                    else if (bits > 8 && bits <=16)
                    {
                        preview_img = host.Make_dng_image(dng_rect(info.fIFD[i]->fImageLength, info.fIFD[i]->fImageWidth), 3, ttShort);
                        sample_size = 2;
                    }

                    printf("samples: %d, bits: %d, max_val: %d, sample_size: %d\n", samples, bits, max_val, sample_size);

                    info.fIFD[i]->ReadImage(host, stream, *preview_img);

                    void* buff_preview_data = malloc(info.fIFD[i]->fImageLength * info.fIFD[i]->fImageWidth * 3 * sample_size);

                    dng_pixel_buffer buf_preview(dng_rect(info.fIFD[i]->fImageLength, info.fIFD[i]->fImageWidth),
                        0,
                        3,
                        ttByte,
                        pcInterleaved,
                    ...
                    preview_img->Get(buf_preview);

                    char main_fn[64] = { 0 };
                    sprintf(main_fn, "main_img_%lld.pnm", i);

                    char header0[64] = { 0 };
                    int len = sprintf(header0, "P6\n%d %d\n%d\n", info.fIFD[i]->fImageWidth, info.fIFD[i]->fImageLength, max_val);
                    if (strlen(header0) % 2 == 1)
                    {
                        len = sprintf(header0, "P6\n%d  %d\n%d\n", info.fIFD[i]->fImageWidth, info.fIFD[i]->fImageLength, max_val);
                    }
                    FILE* main_fp = fopen(main_fn, "wb");
                    if (main_fp != NULL)
                    {
                        fwrite(header0, sizeof(char), strlen(header0), main_fp);
                        fwrite(buff_preview_data, sample_size, info.fIFD[i]->fImageLength * info.fIFD[i]->fImageWidth * 3, main_fp);
                        fclose(main_fp);
                    }

                    free(buff_preview_data);
                }
            }
            ...
            else
            {
                ...
                printf("raw width %d, height %d, pixel size %d, planes: %d, range: %d\n", stage1->Width(), stage1->Height(), stage1->PixelSize(), 
                    stage1->Planes(), stage1->PixelRange());

                void* p_raw = malloc(stage1->Width() * stage1->Height() * stage1->Planes() * stage1->PixelSize());

                dng_pixel_buffer raw_buf(dng_rect(stage1->Height(), stage1->Width()), 0, stage1->Planes(), stage1->PixelType(), pcInterleaved, p_raw);

                stage1->Get(raw_buf);

                char* raw_pnm_fn = "iphone_raw.pnm";
                char* raw_fn = "iphone_raw.raw";

                char header0[64] = { 0 };
                int len = sprintf(header0, "P6\n%d %d\n%d\n", stage1->Width(), stage1->Height(), stage1->PixelRange());
                if (strlen(header0) % 2 == 1)
                {
                    len = sprintf(header0, "P6\n%d  %d\n%d\n", stage1->Width(), stage1->Height(), stage1->PixelRange());
                }

                FILE* raw_fp = fopen(raw_pnm_fn, "wb");
                if (raw_fp != NULL)
                {
                    fwrite(header0, sizeof(char), strlen(header0), raw_fp);
                    fwrite(p_raw, stage1->PixelSize(), stage1->Width() * stage1->Height() * stage1->Planes(), raw_fp);
                    fclose(raw_fp);
                }
                raw_fp = fopen(raw_fn, "wb");
                if (raw_fp != NULL)
                {
                    fwrite(p_raw, stage1->PixelSize(), stage1->Width() * stage1->Height() * stage1->Planes(), raw_fp);
                    fclose(raw_fp);
            ...
            if (hasSemanticMasks)
            {
                ...
                negative->ReadSemanticMasks(host,
                    stream,
                    info);
                uint32_t mask_cnt = negative->NumSemanticMasks();
                printf("has SemanticMasks %d\n", mask_cnt);
                for (uint32_t i = 0; i < mask_cnt; i++)
                {
                    dng_semantic_mask mask(negative->SemanticMask(i));
                    printf("%s\n", mask.fName.Get());
                    dng_rect tblr(mask.fMask->Bounds());
                    printf("top %d, bottom %d, left %d, right %d\n", tblr.t, tblr.b, tblr.l, tblr.r);
                    printf("pixel size = %d, width %d, height %d, planes %d\n", mask.fMask->PixelSize(), mask.fMask->Width(), mask.fMask->Height(), mask.fMask->Planes());
                    printf("pixel type = %d(tiff ttByte == 1), range %d\n", mask.fMask->PixelType(), mask.fMask->PixelRange());

                    void* data = malloc(mask.fMask->PixelSize() * mask.fMask->Width() * mask.fMask->Height());
                    
                    dng_pixel_buffer buffer(tblr,
                        0,
                        mask.fMask->Planes(),
                        mask.fMask->PixelType(),
                        pcPlanar,
                        data);
                    mask.fMask->Get(buffer);
                    
                    std::string fn(mask.fName.Get());
                    ...
                    char header0[64] = { 0 };
                    int len = sprintf(header0, "P5\n%d %d\n%d\n", mask.fMask->Width(), mask.fMask->Height(), 255);
                    if (strlen(header0) % 2 == 1)
                    {
                        len = sprintf(header0, "P5\n%d  %d\n%d\n", mask.fMask->Width(), mask.fMask->Height(), 255);
                    }
                    FILE* mask_fp = fopen(fn.c_str(), "wb");
                    if (mask_fp != NULL)
                    {
                        fwrite(header0, sizeof(char), strlen(header0), mask_fp);
                        fwrite(data, mask.fMask->PixelSize(), mask.fMask->Width() * mask.fMask->Height(), mask_fp);
                        fclose(mask_fp);
                    }
                    free(data);
                }
